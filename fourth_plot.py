...
class MplWidget(FigureCanvasQTAgg):
    def __init__(self, euler_steps, runge_steps, parent=None):
        ...
        self.dy = lambda x,y: -x + np.sqrt(x**2 + 2*y)
    ...
    def accurate_solution(self, x_init, y_init):
        ...
        for i in range(1, 4000 + 1):
            ...
            y_current = y_current + y_delta
            x_current = x_current + h
        self.ax.plot(x_plot, y_runge, color='green', linewidth=2)
        self.ax.set_xlim(-5, 5)
        self.ax.set_ylim(-5, 5)
        self.draw()
...
class TopLevelWindow6(QtWidgets.QMainWindow):
